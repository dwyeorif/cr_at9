...
def binance_price(tk, p_exchange):
    b_ticker = binance.fetch_ticker(tk+'/BUSD')  # Binance current KRW price
...
def binance_usd_price(tk):
    b_ticker = binance.fetch_ticker(tk+'/BUSD')  # Binance current USD price
    price = b_ticker['close']
    return price


def binance_buy(tk, cnt):
    order = binance.create_market_buy_order(tk + '/BUSD', cnt)  # Binance market buy by count
    print(order)


def binance_sell(tk, cnt):
    order = binance.create_market_sell_order(tk + '/BUSD', cnt)  # Binance market sell by count
    print(order)
...
while True:
    try:
        if binance_balance('BUSD') > round(u_count*binance_usd_price(ticker)*1.05) and upbit.get_balance('KRW-' + ticker) > u_count:  # Premium trading
            if get_premium((binance_usd_price(ticker)*p_exchange), upbit_price(ticker), ticker) > (p_standard+p_gap):  # Above standard
                ...
                if binance_balance('BUSD') < round(u_count * binance_usd_price(ticker) * 1.06):  # Reset
        ...
        if upbit.get_balance('KRW') > round(u_mount*1.05) and binance_balance(ticker) > b_count:  # Reverse premium trading
